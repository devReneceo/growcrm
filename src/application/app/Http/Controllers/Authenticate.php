...
class Authenticate extends Controller
{
    ...
    public function resetPassword()
    {
        ...
        //validate code
        if (
            \App\Models\User::Where('forgot_password_token', request('token'))
            ...
            request()
                ->session()
                ->flash(
                    'error-notification-longer',
                    __('lang.url_expired_or_invalid')
                );
            //redirect
    ...
    public function apilogInAction(Request $request)
    {
        ...
        if (Auth::attempt($credentials, $remember)) {
            ...
            // if (
            //     auth()->user()->is_client &&
            //     config('system.settings_clients_app_login') != 'enabled'
            // ) {
            ...
            //     abort(409, __('lang.account_has_been_suspended'));
            // }
            echo json_encode(['status' => 'success', 'user' => auth()->user()]);
        } else {
            //login failed message
            //abort(409, __('lang.invalid_login_details'));
            echo json_encode([
                'status' => 'faild',
                'error' => __('lang.invalid_login_details'),
            ]);
        }
    }
    /**
    ...
    public function forgotPasswordAction()
    {
        ...
        $user->forgot_password_token_expiry = \Carbon\Carbon::now()->addHours(
            3
        );
        $user->save();
        ...
        if (
            $user->type == 'client' &&
            config('system.settings_clients_disable_email_delivery') ==
                'enabled'
        ) {
            abort(409, __('lang.clients_disabled_login_error'));
        } else {
        ...
        request()
            ...
            ->flash(
                ...
                __('lang.password_reset_email_sent')
            );
    ...
    public function resetPasswordAction()
    {
        ...
        request()
            ...
            ->flash(
                'success-notification-longer',
                __('lang.password_reset_success')
            );
        //back to login
        $jsondata['redirect_url'] = url('login');
        return response()->json($jsondata);
