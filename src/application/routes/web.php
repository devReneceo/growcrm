...
Route::group(['prefix' => 'app'], function () {
    Route::get('daily-report', [ProgramController::class, 'dailyReport']);
...
Route::group(['prefix' => 'clients'], function () {
    ...
    Route::get('/{client}/client-details', 'Clients@details')->where(
        'client',
        '[0-9]+'
    );
    Route::post('/{client}/client-details', 'Clients@updateDescription')->where(
        'client',
        '[0-9]+'
    );
    ...
    //dynamic load
    Route::any('/{client}/{section}', 'Clients@showDynamic')->where([
        'client' => '[0-9]+',
        'section' =>
...
Route::group(['prefix' => 'user'], function () {
    ...
    Route::put('/updatepassword', 'User@updatePasswordAction')->middleware([
        'demoModeCheck',
    ]);
    Route::get('/updatenotifications', 'User@updateNotifications');
    Route::put(
        '/updatenotifications',
        'User@updateNotificationsAction'
    )->middleware(['demoModeCheck']);
    Route::post('/updatelanguage', 'User@updateLanguage')->middleware([
        'demoModeCheck',
    ]);
    Route::get('/updatetheme', 'User@updateTheme');
    Route::put('/updatetheme', 'User@updateThemeAction')->middleware([
        'demoModeCheck',
    ]);
});
...
Route::group(['prefix' => 'invoices'], function () {
    ...
    Route::get('/{invoice}/clone', 'Invoices@createClone')->where(
        'invoice',
        '[0-9]+'
    );
    Route::post('/{invoice}/clone', 'Invoices@storeClone')->where(
        'invoice',
        '[0-9]+'
    );
    Route::get('/{invoice}/stop-recurring', 'Invoices@stopRecurring')->where(
        'invoice',
        '[0-9]+'
    );
    Route::get('/{invoice}/attach-project', 'Invoices@attachProject')->where(
        'invoice',
        '[0-9]+'
    );
    Route::post(
        '/{invoice}/attach-project',
        'Invoices@attachProjectUpdate'
    )->where('invoice', '[0-9]+');
    Route::get('/{invoice}/detach-project', 'Invoices@dettachProject')->where(
        'invoice',
        '[0-9]+'
    );
    Route::get('/{invoice}/email-client', 'Invoices@emailClient')->where(
        'invoice',
        '[0-9]+'
    );
    Route::get('/{invoice}/download-pdf', 'Invoices@downloadPDF')->where(
        'invoice',
        '[0-9]+'
    );
    Route::get(
        '/{invoice}/recurring-settings',
        'Invoices@recurringSettings'
    )->where('invoice', '[0-9]+');
    Route::post(
        '/{invoice}/recurring-settings',
        'Invoices@recurringSettingsUpdate'
    )->where('invoice', '[0-9]+');
    Route::get('/{invoice}/edit-invoice', 'Invoices@show')
        ->where('invoice', '[0-9]+')
        ->middleware(['invoicesMiddlewareEdit', 'invoicesMiddlewareShow']);
    Route::post('/{invoice}/edit-invoice', 'Invoices@saveInvoice')->where(
        'invoice',
        '[0-9]+'
    );
    Route::get('/{invoice}/pdf', 'Invoices@show')
        ->where('invoice', '[0-9]+')
        ->middleware(['invoicesMiddlewareShow']);
    Route::get('/{invoice}/publish', 'Invoices@publishInvoice')
        ->where('invoice', '[0-9]+')
        ->middleware(['invoicesMiddlewareEdit', 'invoicesMiddlewareShow']);
    Route::get('/{invoice}/resend', 'Invoices@resendInvoice')
        ->where('invoice', '[0-9]+')
        ->middleware(['invoicesMiddlewareEdit', 'invoicesMiddlewareShow']);
    Route::get('/{invoice}/stripe-payment', 'Invoices@paymentStripe')->where(
        'invoice',
        '[0-9]+'
    );
    Route::get('/{invoice}/paypal-payment', 'Invoices@paymentPaypal')->where(
        'invoice',
        '[0-9]+'
    );
    Route::get('/timebilling/{project}/', 'Timebilling@index')->where(
        'project',
        '[0-9]+'
    );
    Route::get(
        '/{invoice}/razorpay-payment',
        'Invoices@paymentRazorpay'
    )->where('invoice', '[0-9]+');
    Route::get('/{invoice}/mollie-payment', 'Invoices@paymentMollie')->where(
        'invoice',
        '[0-9]+'
    );
});
...
Route::group(['prefix' => 'subscriptions'], function () {
    ...
    Route::post('/delete', 'Subscriptions@destroy')->middleware([
        'demoModeCheck',
    ]);
    ...
    Route::get(
        '/{subscription}/pay',
        'Subscriptions@setupStripePayment'
    )->where('subscription', '[0-9]+');
    Route::get('/{subscription}/cancel', 'Subscriptions@cancelSubscription')
        ->where('subscription', '[0-9]+')
        ->middleware(['demoModeCheck']);
});
...
Route::group(['prefix' => 'estimates'], function () {
    ...
    Route::get('/{estimate}/attach-project', 'Estimates@attachProject')->where(
        'estimate',
        '[0-9]+'
    );
    Route::post(
        '/{estimate}/attach-project',
        'Estimates@attachProjectUpdate'
    )->where('invoice', '[0-9]+');
    Route::get('/{estimate}/detach-project', 'Estimates@dettachProject')->where(
        'estimate',
        '[0-9]+'
    );
    Route::get('/{estimate}/email-client', 'Estimates@emailClient')->where(
        'estimate',
        '[0-9]+'
    );
    Route::get(
    ...
    Route::get('/{estimate}/change-status', 'Estimates@changeStatus')->where(
        'estimate',
        '[0-9]+'
    );
    Route::post(
        '/{estimate}/change-status',
        'Estimates@changeStatusUpdate'
    )->where('estimate', '[0-9]+');
    Route::get('/{estimate}/edit-estimate', 'Estimates@show')
        ->where('estimate', '[0-9]+')
        ->middleware(['estimatesMiddlewareEdit', 'estimatesMiddlewareShow']);
    Route::post('/{estimate}/edit-estimate', 'Estimates@saveEstimate')->where(
        'estimate',
        '[0-9]+'
    );
    Route::get('/{estimate}/pdf', 'Estimates@show')
        ->where('estimate', '[0-9]+')
        ->middleware(['estimatesMiddlewareShow']);
    Route::get('/{estimate}/publish', 'Estimates@publishEstimate')
        ->where('estimate', '[0-9]+')
        ->middleware(['estimatesMiddlewareEdit', 'estimatesMiddlewareShow']);
    Route::get(
        '/{estimate}/publish-revised',
        'Estimates@publishRevisedEstimate'
    )
        ->where('estimate', '[0-9]+')
        ->middleware(['estimatesMiddlewareEdit', 'estimatesMiddlewareShow']);
    Route::get('/{estimate}/resend', 'Estimates@resendEstimate')
        ->where('estimate', '[0-9]+')
        ->middleware(['estimatesMiddlewareEdit', 'estimatesMiddlewareShow']);
    Route::get('/{estimate}/accept', 'Estimates@acceptEstimate')->where(
        'estimate',
        '[0-9]+'
    );
    Route::get('/{estimate}/decline', 'Estimates@declineEstimate')->where(
        'estimate',
        '[0-9]+'
    );
    Route::get(
    ...
    Route::post(
        '/{estimate}/convert-to-invoice',
        'Estimates@convertToInvoiceAction'
    )->where('invoice', '[0-9]+');
    Route::get('/{estimate}/clone', 'Estimates@createClone')->where(
        'estimate',
        '[0-9]+'
    );
    Route::post('/{estimate}/clone', 'Estimates@storeClone')->where(
        'estimate',
        '[0-9]+'
    );
});
...
Route::group(['prefix' => 'expenses'], function () {
    ...
    Route::get('/{expense}/attach-dettach', 'Expenses@attachDettach')->where(
        'invoice',
        '[0-9]+'
    );
    Route::post(
        '/{expense}/attach-dettach',
        'Expenses@attachDettachUpdate'
    )->where('invoice', '[0-9]+');
    Route::get('/change-category', 'Expenses@changeCategory');
    Route::post('/change-category', 'Expenses@changeCategoryUpdate');
    Route::get(
        '/{expense}/create-new-invoice',
        'Expenses@createNewInvoice'
    )->where('expense', '[0-9]+');
    Route::post(
        '/{expense}/create-new-invoice',
        'Expenses@createNewInvoice'
    )->where('expense', '[0-9]+');
    Route::get('/{expense}/add-to-invoice', 'Expenses@addToInvoice')->where(
        'expense',
        '[0-9]+'
    );
    Route::post('/{expense}/add-to-invoice', 'Expenses@addToInvoice')->where(
        'expense',
        '[0-9]+'
    );
...
Route::group(['prefix' => 'projects'], function () {
    ...
    Route::get('/{project}/change-status', 'Projects@changeStatus')->where(
        'project',
        '[0-9]+'
    );
    Route::post(
        '/{project}/change-status',
        'Projects@changeStatusUpdate'
    )->where('project', '[0-9]+');
    Route::get('/{project}/project-details', 'Projects@details')->where(
        'project',
        '[0-9]+'
    );
    Route::post(
    ...
    Route::put('/{project}/stop-all-timers', 'Projects@stopAllTimers')->where(
        'project',
        '[0-9]+'
    );
    Route::put('/{project}/archive', 'Projects@archive')->where(
        'project',
        '[0-9]+'
    );
    Route::put('/{project}/activate', 'Projects@activate')->where(
        'project',
        '[0-9]+'
    );
    Route::get('/{project}/clone', 'Projects@createClone')->where(
        'project',
        '[0-9]+'
    );
    Route::post('/{project}/clone', 'Projects@storeClone')->where(
        'project',
        '[0-9]+'
    );
    Route::get('/prefill-project', 'Projects@prefillProject');
    Route::get('/{project}/progress', 'Projects@changeProgress')->where(
        'project',
        '[0-9]+'
    );
    Route::post('/{project}/progress', 'Projects@changeProgressUpdate')->where(
        'project',
        '[0-9]+'
    );
    Route::get(
        '/{project}/change-cover-image',
        'Projects@changeCoverImage'
    )->where('project', '[0-9]+');
    Route::post(
        '/{project}/change-cover-image',
        'Projects@changeCoverImageUpdate'
    )->where('project', '[0-9]+');
    Route::get('/{project}/assigned', 'Projects@assignedUsers')->where(
        'project',
        '[0-9]+'
    );
    Route::put('/{project}/assigned', 'Projects@assignedUsersUpdate')->where(
        'project',
        '[0-9]+'
    );

    //dynamic load
    Route::any('/{project}/{section}', 'Projects@showDynamic')->where([
        'project' => '[0-9]+',
        'section' =>
            'details|comments|files|tasks|invoices|payments|timesheets|expenses|estimates|milestones|tickets|notes',
    ]);
});
...
Route::group(['prefix' => 'tasks'], function () {
    ...
    Route::any('/timer/{id}/stopall', 'Tasks@timerStopAll')->where(
        'id',
        '[0-9]+'
    );
    Route::post('/delete', 'Tasks@destroy')->middleware(['demoModeCheck']);
    Route::post('/{task}/toggle-status', 'Tasks@toggleStatus')->where(
        'task',
        '[0-9]+'
    );
    Route::post('/{task}/update-description', 'Tasks@updateDescription')->where(
        'task',
        '[0-9]+'
    );
    Route::post('/{task}/attach-files', 'Tasks@attachFiles')->where(
        'task',
        '[0-9]+'
    );
    Route::delete(
        '/delete-attachment/{uniqueid}',
        'Tasks@deleteAttachment'
    )->middleware(['demoModeCheck']);
    Route::get('/download-attachment/{uniqueid}', 'Tasks@downloadAttachment');
    Route::post('/{task}/post-comment', 'Tasks@storeComment')->where(
        'task',
        '[0-9]+'
    );
    Route::delete('/delete-comment/{commentid}', 'Tasks@deleteComment')->where(
        'commentid',
        '[0-9]+'
    );
    Route::post('/{task}/update-title', 'Tasks@updateTitle')->where(
        'task',
        '[0-9]+'
    );
    Route::post('/{task}/add-checklist', 'Tasks@storeChecklist')->where(
        'task',
        '[0-9]+'
    );
    Route::post(
    ...
    Route::post(
        '/toggle-checklist-status/{checklistid}',
        'Tasks@toggleChecklistStatus'
    )->where('checklistid', '[0-9]+');
    Route::post('/{task}/update-start-date', 'Tasks@updateStartDate')->where(
        'task',
        '[0-9]+'
    );
    Route::post('/{task}/update-due-date', 'Tasks@updateDueDate')->where(
        'task',
        '[0-9]+'
    );
    Route::post('/{task}/update-status', 'Tasks@updateStatus')->where(
        'task',
        '[0-9]+'
    );
    Route::post('/{task}/update-priority', 'Tasks@updatePriority')->where(
        'task',
        '[0-9]+'
    );
    Route::post('/{task}/update-visibility', 'Tasks@updateVisibility')->where(
        'task',
        '[0-9]+'
    );
    Route::post('/{task}/update-milestone', 'Tasks@updateMilestone')->where(
        'task',
        '[0-9]+'
    );
    Route::post('/{task}/update-assigned', 'Tasks@updateAssigned')->where(
        'task',
        '[0-9]+'
    );
    Route::post('/{task}/update-tags', 'Tasks@updateTags')->where(
        'task',
        '[0-9]+'
    );
    Route::post('/update-position', 'Tasks@updatePosition');
    Route::any('/v/{task}/{slug}', 'Tasks@index')->where('task', '[0-9]+');
    Route::post('/{task}/update-custom', 'Tasks@updateCustomFields')->where(
        'task',
        '[0-9]+'
    );
    ...
    Route::get('/{task}/recurring-settings', 'Tasks@recurringSettings')->where(
        'task',
        '[0-9]+'
    );
    Route::post(
        '/{task}/recurring-settings',
        'Tasks@recurringSettingsUpdate'
    )->where('task', '[0-9]+');
    Route::get('/{task}/stop-recurring', 'Tasks@stopRecurring')->where(
        'task',
        '[0-9]+'
    );

    //card tabs
    Route::get('/content/{task}/show-main', 'Tasks@show')->where(
        'lead',
        '[0-9]+'
    );
    Route::get(
        '/content/{task}/show-customfields',
        'Tasks@showCustomFields'
    )->where('task', '[0-9]+');
    Route::get(
        '/content/{task}/edit-customfields',
        'Tasks@editCustomFields'
    )->where('task', '[0-9]+');
    Route::post(
        '/content/{task}/edit-customfields',
        'Tasks@updateCustomFields'
    )->where('task', '[0-9]+');
    Route::get('/content/{task}/show-mynotes', 'Tasks@showMyNotes')->where(
        'task',
        '[0-9]+'
    );
    Route::get('/content/{task}/create-mynotes', 'Tasks@createMyNotes')->where(
        'task',
        '[0-9]+'
    );
    Route::get('/content/{task}/edit-mynotes', 'Tasks@editMyNotes')->where(
        'task',
        '[0-9]+'
    );
    Route::delete(
        '/content/{task}/delete-mynotes',
        'Tasks@deleteMyNotes'
    )->where('task', '[0-9]+');
    Route::post('/content/{task}/edit-mynotes', 'Tasks@updateMyNotes')->where(
        'task',
        '[0-9]+'
    );
});
...
Route::group(['prefix' => 'leads'], function () {
    ...
    Route::get('/{lead}/change-status', 'Leads@changeStatus')->where(
        'lead',
        '[0-9]+'
    );
    Route::post('/{lead}/change-status', 'Leads@changeStatusUpdate')->where(
        'lead',
        '[0-9]+'
    );
    Route::post('/{lead}/update-description', 'Leads@updateDescription')->where(
        'lead',
        '[0-9]+'
    );
    Route::post('/{lead}/attach-files', 'Leads@attachFiles')->where(
        'lead',
        '[0-9]+'
    );
    ...
    Route::post('/{lead}/update-title', 'Leads@updateTitle')->where(
        'lead',
        '[0-9]+'
    );
    Route::post('/{lead}/post-comment', 'Leads@storeComment')->where(
        'lead',
        '[0-9]+'
    );
    Route::delete('/delete-comment/{commentid}', 'Leads@deleteComment')->where(
        'commentid',
        '[0-9]+'
    );
    Route::post('/{lead}/add-checklist', 'Leads@storeChecklist')->where(
        'lead',
        '[0-9]+'
    );
    Route::post(
        '/update-checklist/{checklistid}',
        'Leads@updateChecklist'
    )->where('checklistid', '[0-9]+');
    Route::delete(
        '/delete-checklist/{checklistid}',
        'Leads@deleteChecklist'
    )->where('checklistid', '[0-9]+');
    Route::post(
        '/toggle-checklist-status/{checklistid}',
        'Leads@toggleChecklistStatus'
    )->where('checklistid', '[0-9]+');
    Route::post('/{lead}/update-date-added', 'Leads@updateDateAdded')->where(
        'lead',
        '[0-9]+'
    );
    Route::post('/{lead}/update-name', 'Leads@updateName')->where(
        'lead',
        '[0-9]+'
    );
    Route::post('/{lead}/update-value', 'Leads@updateValue')->where(
        'lead',
        '[0-9]+'
    );
    Route::post('/{lead}/update-status', 'Leads@updateStatus')->where(
        'lead',
        '[0-9]+'
    );
    Route::post('/{lead}/update-category', 'Leads@updateCategory')->where(
        'lead',
        '[0-9]+'
    );
    Route::post('/{lead}/update-contacted', 'Leads@updateContacted')->where(
        'lead',
        '[0-9]+'
    );
    Route::post('/{lead}/update-phone', 'Leads@updatePhone')->where(
        'lead',
        '[0-9]+'
    );
    Route::post('/{lead}/update-email', 'Leads@updateEmail')->where(
        'lead',
        '[0-9]+'
    );
    Route::post('/{lead}/update-source', 'Leads@updateSource')->where(
        'lead',
        '[0-9]+'
    );
    Route::post(
        '/{lead}/update-organisation',
        'Leads@updateOrganisation'
    )->where('lead', '[0-9]+');
    Route::post('/{lead}/update-assigned', 'Leads@updateAssigned')->where(
        'lead',
        '[0-9]+'
    );
    Route::post('/{lead}/update-tags', 'Leads@updateTags')->where(
        'lead',
        '[0-9]+'
    );
    Route::post('/update-position', 'Leads@updatePosition');
    Route::post('/{lead}/convert-lead', 'Leads@convertLead')->where(
        'lead',
        '[0-9]+'
    );
    Route::any('/v/{lead}/{slug}', 'Leads@index')->where('lead', '[0-9]+');
    Route::post('/{lead}/update-custom', 'Leads@updateCustomFields')->where(
        'lead',
        '[0-9]+'
    );
    ...
    //card tabs
    Route::get('/content/{lead}/show-main', 'Leads@show')->where(
        'lead',
        '[0-9]+'
    );
    Route::get(
        '/content/{lead}/show-organisation',
        'Leads@showOrganisation'
    )->where('lead', '[0-9]+');
    Route::get(
        '/content/{lead}/edit-organisation',
        'Leads@editOrganisation'
    )->where('lead', '[0-9]+');
    Route::post(
        '/content/{lead}/edit-organisation',
        'Leads@updateOrganisation'
    ...
    Route::get(
        '/content/{lead}/show-customfields',
        'Leads@showCustomFields'
    )->where('lead', '[0-9]+');
    Route::get(
        '/content/{lead}/edit-customfields',
        'Leads@editCustomFields'
    )->where('lead', '[0-9]+');
    Route::post(
        '/content/{lead}/edit-customfields',
        'Leads@updateCustomFields'
    )->where('lead', '[0-9]+');
    Route::get('/content/{lead}/show-mynotes', 'Leads@showMyNotes')->where(
        'lead',
        '[0-9]+'
    );
    Route::get('/content/{lead}/create-mynotes', 'Leads@createMyNotes')->where(
        'lead',
        '[0-9]+'
    );
    Route::get('/content/{lead}/edit-mynotes', 'Leads@editMyNotes')->where(
        'lead',
        '[0-9]+'
    );
    Route::delete(
        '/content/{lead}/delete-mynotes',
        'Leads@deleteMyNotes'
    )->where('lead', '[0-9]+');
    Route::post('/content/{lead}/edit-mynotes', 'Leads@updateMyNotes')->where(
        'lead',
        '[0-9]+'
    );
    Route::get('/content/{lead}/show-logs', 'Leads@showLogs')->where(
        'lead',
        '[0-9]+'
    );
    Route::get('/content/{lead}/edit-logs', 'Leads@editLogs')->where(
        'lead',
        '[0-9]+'
    );
    Route::post('/content/{lead}/edit-logs', 'Leads@updateLogs')->where(
        'lead',
        '[0-9]+'
    );
});
...
Route::group(['prefix' => 'tickets'], function () {
    ...
    Route::post('/{ticket}/postreply', 'Tickets@storeReply')->where(
        'x',
        '[0-9]+'
    );
...
Route::group(['prefix' => 'files'], function () {
    ...
    Route::post('/{file}/rename', 'Files@renameFile')->middleware([
        'demoModeCheck',
    ]);
});
...
Route::group(['prefix' => 'categories'], function () {
    ...
    Route::get('/{category}/team', 'Categories@showTeam')->where(
        'category',
        '[0-9]+'
    );
    Route::put('/{category}/team', 'Categories@updateTeam')->where(
        'category',
        '[0-9]+'
    );
});
...
Route::group(['prefix' => 'settings/projects'], function () {
    ...
    Route::put('/general', 'Settings\Projects@updateGeneral')->middleware([
        'demoModeCheck',
    ]);
    Route::get('/client', 'Settings\Projects@clientPermissions');
    Route::put(
        '/client',
        'Settings\Projects@updateClientPermissions'
    )->middleware(['demoModeCheck']);
    Route::get('/staff', 'Settings\Projects@staffPermissions');
    Route::put(
        '/staff',
        'Settings\Projects@updateStaffPermissions'
    )->middleware(['demoModeCheck']);
});
...
Route::group(['prefix' => 'settings/subscriptions'], function () {
    ...
    Route::put('/', 'Settings\Subscriptions@update')->middleware([
        'demoModeCheck',
    ]);
});
...
Route::group(['prefix' => 'settings/leads'], function () {
    ...
    Route::put('/statuses', 'Settings\Leads@updateStatuses')->middleware([
        'demoModeCheck',
    ]);
    Route::get('/statuses/{id}/edit', 'Settings\Leads@editStatus')->where(
        'lead',
        '[0-9]+'
    );
    Route::put('/statuses/{id}', 'Settings\Leads@updateStatus')
        ->where('lead', '[0-9]+')
        ->middleware(['demoModeCheck']);
    ...
    Route::put('/move/{id}', 'Settings\Leads@updateMove')->where(
        'id',
        '[0-9]+'
    );
    Route::delete('/statuses/{id}', 'Settings\Leads@destroyStatus')
        ->where('id', '[0-9]+')
        ->middleware(['demoModeCheck']);
    Route::post(
        '/update-stage-positions',
        'Settings\Leads@updateStagePositions'
    );
});
...
Route::group(['prefix' => 'settings/milestones'], function () {
    ...
    Route::put('/settings', 'Settings\Milestones@update')->middleware([
        'demoModeCheck',
    ]);
    Route::get('/default', 'Settings\Milestones@categories');
    Route::get('/create', 'Settings\Milestones@create');
    Route::post('/create', 'Settings\Milestones@storeCategory')->middleware([
        'demoModeCheck',
    ]);
    Route::get('/{id}/edit', 'Settings\Milestones@editCategory')->where(
        'id',
        '[0-9]+'
    );
    Route::put('/{id}', 'Settings\Milestones@updateCategory')
        ->where('id', '[0-9]+')
        ->middleware(['demoModeCheck']);
    Route::post(
        '/update-positions',
        'Settings\Milestones@updateCategoryPositions'
    );
    Route::delete('/{id}', 'Settings\Milestones@destroy')
        ->where('id', '[0-9]+')
        ->middleware(['demoModeCheck']);
});
...
Route::group(['prefix' => 'settings/knowledgebase'], function () {
    ...
    Route::put('/settings', 'Settings\Knowledgebase@update')->middleware([
        'demoModeCheck',
    ]);
    Route::get('/default', 'Settings\Knowledgebase@categories');
    Route::get('/create', 'Settings\Knowledgebase@create');
    Route::post('/create', 'Settings\Knowledgebase@storeCategory')->middleware([
        'demoModeCheck',
    ]);
    Route::get('/{id}/edit', 'Settings\Knowledgebase@editCategory')->where(
        'id',
        '[0-9]+'
    );
    Route::put('/{id}', 'Settings\Knowledgebase@updateCategory')
        ->where('id', '[0-9]+')
        ->middleware(['demoModeCheck']);
    Route::post(
        '/update-positions',
        'Settings\Knowledgebase@updateCategoryPositions'
    );
    Route::delete('/{id}', 'Settings\Knowledgebase@destroy')
        ->where('id', '[0-9]+')
        ->middleware(['demoModeCheck']);
});
...
Route::group(['prefix' => 'settings/tasks'], function () {
    ...
    Route::put('/statuses', 'Settings\Tasks@updateStatuses')->middleware([
        'demoModeCheck',
    ]);
    Route::get('/statuses/{id}/edit', 'Settings\Tasks@editStatus')->where(
        'task',
        '[0-9]+'
    );
    Route::put('/statuses/{id}', 'Settings\Tasks@updateStatus')
        ->where('task', '[0-9]+')
        ->middleware(['demoModeCheck']);
    ...
    Route::put('/move/{id}', 'Settings\Tasks@updateMove')->where(
        'id',
        '[0-9]+'
    );
    Route::delete('/statuses/{id}', 'Settings\Tasks@destroyStatus')
        ->where('id', '[0-9]+')
        ->middleware(['demoModeCheck']);
    Route::post(
        '/update-stage-positions',
        'Settings\Tasks@updateStagePositions'
    );
});
...
Route::group(['prefix' => 'settings/email'], function () {
    ...
    Route::put('/general', 'Settings\Email@updateGeneral')->middleware([
        'demoModeCheck',
    ]);
    Route::get('/smtp', 'Settings\Email@smtp')->middleware(['demoModeCheck']);
    Route::put('/smtp', 'Settings\Email@updateSMTP')->middleware([
        'demoModeCheck',
    ]);
    Route::get('/templates', 'Settings\Emailtemplates@index');
    Route::get('/templates/{id}', 'Settings\Emailtemplates@show')->where(
        'id',
        '[0-9]+'
    );
    Route::post('/templates/{id}', 'Settings\Emailtemplates@update')
        ->where('id', '[0-9]+')
        ->middleware(['demoModeCheck']);
    Route::get('/testemail', 'Settings\Email@testEmail')->middleware([
        'demoModeCheck',
    ]);
    Route::post('/testemail', 'Settings\Email@testEmailAction')->middleware([
        'demoModeCheck',
    ]);
    Route::get('/testsmtp', 'Settings\Email@testSMTP')->middleware([
        'demoModeCheck',
    ]);
    Route::get('/queue', 'Settings\Email@queueShow')->where('id', '[0-9]+');
    Route::get('/queue/{id}', 'Settings\Email@queueRead')->where(
        'id',
        '[0-9]+'
    );
    Route::delete('/queue/{id}', 'Settings\Email@queueDelete')->where(
        'id',
        '[0-9]+'
    );
    ...
    Route::delete('/log/{id}', 'Settings\Email@logDelete')->where(
        'id',
        '[0-9]+'
    );
...
Route::post('/settings/roles', 'Settings\Roles@Store')->middleware([
    'demoModeCheck',
]);
...
Route::group(['prefix' => 'settings/logos'], function () {
    ...
    Route::put('/uploadlogo', 'Settings\Logos@updateLogo')->middleware([
        'demoModeCheck',
    ]);
});
...
Route::group(['prefix' => 'app/settings'], function () {
    Route::get('/{any}', 'Settings\Dynamic@showDynamic')->where([
        'any' => '.*',
    ]);
});
...
Route::group(['prefix' => 'settings/subscriptions'], function () {
    ...
    Route::post('/plans', 'Settings\Subscriptions@storePlan')->middleware([
        'demoModeCheck',
    ]);
    Route::put('/plans', 'Settings\Subscriptions@updatePlan')->middleware([
        'demoModeCheck',
    ]);
});
...
Route::group(['prefix' => 'settings/customfields'], function () {
    ...
    Route::delete('/{id}', 'Settings\Customfields@destroy')->where(
        'id',
        '[0-9]+'
    );
    ...
    Route::put(
        ...
        'Settings\Customfields@updateStandardFormRequired'
    );
    Route::post(
        '/update-standard-form-positions',
        'Settings\Customfields@updateFieldPositions'
    );
    Route::put(
        '/standard-form-display-settings',
        'Settings\Customfields@updateDisplaySettings'
    );
});
...
Route::group(['prefix' => 'events'], function () {
    ...
    Route::get('/{id}/mark-read-my-event', 'Events@markMyEventRead')->where(
        'id',
        '[0-9]+'
    );
...
Route::group(['prefix' => 'updating'], function () {
    ...
    Route::get(
        ...
        'Updating\Action@showUpdatingCurrencySetting'
    );
    Route::put(
        '/update-currency-settings',
        'Updating\Action@updateUpdatingCurrencySetting'
    );
});
...
Route::group(['prefix' => 'templates/projects'], function () {
    Route::any('/search', 'Templates\Projects@index')->middleware([
        'projectTemplatesGeneral',
    ]);
    Route::post('/delete', 'Templates\Projects@destroy')->middleware([
        'demoModeCheck',
    ]);
    Route::get(
        '/{project}/project-details',
        'Templates\Projects@details'
    )->middleware(['projectTemplatesGeneral']);
    Route::post(
        '/{project}/project-details',
        'Templates\Projects@updateDescription'
    );
    //dynamic load
    Route::any('/{project}/{section}', 'Templates\Projects@showDynamic')
        ->where([
            'project' => '-[0-9]+',
            'section' => 'details|files|tasks|milestones',
        ])
        ->middleware(['projectTemplatesGeneral']);
});
Route::resource('templates/projects', 'Templates\Projects')->middleware([
    'projectTemplatesGeneral',
]);
...
Route::post('/appwebmail/send', 'Webmail\Compose@send')->middleware([
    'demoModeCheck',
]);
